...
export_plot <- function(
  ...
  if (!(is.null(output))) {
    output_f = ifelse(grepl(format, output, fixed = TRUE), output, sprintf(paste0("%s.",format), output))
...
visualize_tree <- function(
  ...
  if (flip_nodes == TRUE) {
    if (!is.null(node1) || !is.null(node2)) {
      plot <- ggtree::flip(ggtree(tree_obj, width = tree_linewidth, layout = form), node1, node2)
    } else {
      stop(
  ...
  if (!is.null(highlight_nodes)) {
    if (!is.vector(highlight_nodes)) {
      stop("highlight_nodes should be a vector.")
        ...
        stop("You should choose either highlight_nodes or highlight_clades. Both of them cannot be enabled at the same time.")
    }
    
    # Handle colors for highlighting
    if (is.null(highlight_colors)) {
      highlight_colors <- setNames(
        sample(rainbow(length(highlight_nodes))),
        length(highlight_nodes)
      )
    } else if (length(highlight_colors) != length(highlight_nodes)) {
    ...
    highlight <- data.frame(
      ...
      Color = sapply(names(highlight_nodes), function(group) {
        if (is.null(highlight_colors) || is.na(highlight_colors[group]) || is.null(highlight_colors[group]) ) {
    ...
    plot <- plot +
      geom_hilight(
        ...
        if (is.null(highlight_colors)) {
            warning("No colors provided for highlighting. Using black as default color.")
        }
    
    # Create groups for highlighting
    groups <- list()
    for (group_name in names(highlight_clades)) {
    ...
    # Create highlight data frame with default black for any missing colors
    highlight <- data.frame(
      Group = names(highlight_clades),
      Label = highlight_clades,
      Color = sapply(names(highlight_clades), function(group) {
        if (is.null(highlight_colors) || is.na(highlight_colors[group])) {
          "black"  # Default color if no mapping exists
        } else {
          highlight_colors[group]
        }
      ...
      tree_obj <- treeio::as.treedata(
          as_tibble(tree_obj) %>%
            mutate(
              ...
              bs_group = factor(
                case_when(  # Add grouping for legend
  ...
  if (is.null(highlight_nodes) && is.null(highlight_clades)) {
    if (tiplabels == TRUE) {
    ...
    if (!is.null(color) || !is.null(shape)) {
      ...
      if (!is.null(taxon_group_separator)) {
        ...
        } else if ( taxon_group_field == 2 ) { # Second field in tip label corresponds to the taxon name (OTHER_INFO|taxon_group_separator|TAXON_NAME)
            taxa_dict <- lapply(tree_obj@phylo$tip.label, function(label) {
              list(tip_label = label, group = sub(paste0(taxon_group_separator,".*"), "", label))
            })
        } else {
