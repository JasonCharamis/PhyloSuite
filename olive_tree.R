...
draw_single_tree <- function ( tree, node1=NULL, node2=NULL, reference1=NULL, reference2=NULL, bootstrap_legend=TRUE, ... ) {
  ...
  ggtree(x) + geom_star(aes(x,subset=isTip,starshape=shape, fill=species.x), size=3, show.legend = F) +
    ...
    geom_highlight(node=c( node1, node2), fill=c("rosybrown1","wheat1"), alpha=0.3, extend=0.10, linetype=23, colour="black") +
